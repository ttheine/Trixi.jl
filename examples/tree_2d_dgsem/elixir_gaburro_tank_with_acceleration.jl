...
mesh = TreeMesh(coordinates_min, coordinates_max,
                ...
                n_cells_max=300_000, periodicity=(false,false))
...
function save_my_plot_density(plot_data, variable_names;
    ...
    Plots.plot(alpha_rho_data, 
               clim=(0.0,1000.0), c=:jet1,
               title=title,titlefontsize=9, 
               dpi=300,
               )
    #Plots.plot!(getmesh(plot_data),linewidth=0.4)
    f1(x) = -0.5098 * x + 0.7549 
    Plots.plot!(f1,0.0,1.0,lc=:black)
    
    # Determine filename and save plot
    filename = joinpath("out", @sprintf("solution_movingTank_%06d.png", timestep))
